...
# Function to sort array that will be reused in both exercises with complexity n*log(n)
    ...
    if len(my_list) > 1:
        ...
        while left_index < len(left) and right_index < len(right):
            if left[left_index] <= right[right_index]:
                my_list[index] = left[left_index]
                left_index += 1
            else:
                my_list[index] = right[right_index]
                right_index += 1
            index += 1
...
# Binary search with complecity log(n)
...
def matchPanAndCover(pan, cover):
    ...
    for p in range(len(pan)):
        ...
        if pan_value == mid_cover_value and is_mid_cover_value_used is False:
            ...
            index = binarySearch(cover_left, 0, len(cover_left) - 1, pan_value)
            if index >= 0:
            ...
            index = binarySearch(cover_right, 0, len(cover_right) - 1, pan_value)
...
# Binary Search to get the lower index or the index value when exist the value O(log(n))
...
def map_lower_and_higher_nearest(array, number_of_nears, lower_index, higher_index, value):  # O(n^2)
    result_array = []
...
def getClosestValues(numbers_list, number_of_nears, value):
    if len(numbers_list) <= number_of_nears:
        number_of_nears = len(numbers_list)
    if len(numbers_list) > 0:
        new_list = [*numbers_list]
        mergeSort(new_list)  # O(n*log(n))
        lower_index = search_lower_index(new_list, 0, len(new_list) - 1, value)  # O(log(n))
        higher_index = lower_index + 1
        result = map_lower_and_higher_nearest(new_list, number_of_nears, lower_index, higher_index, value)  # O(n^2)
        print(f"The {k} values closes to V={value} are: ", result)
    else:
        print("Error: Void array")
...
getClosestValues(entrada, k, V)  # Complexity is O(n^2*log(n))
